...
dist_con = max(max(R - dist'));      % Calculates Distance constraint between objects
acc_con  = max(max(acc' - d2_max));  % Calculates Acceleration magnitude constraint
vel_con  = max(max(vel' - d1_max));  % Calculates Velocity magnitude constraint
